...
int getProcInfo(int pid, struct processInfo* pinfo){
	...
	for(p = ptable.proc; p< &ptable.proc[NPROC]; p++){
		if(p->pid == pid && p->state !=UNUSED){
			...
			pinfo->nfd = open_files;

			if(p->pid == 1){ //init process got no parent
				pinfo->ppid = 0; //by convention init process parent id is 0;

			}else{
				if(p->parent) //got parent;
	...
	release(&ptable.lock);
	return -1; // Error: did not found the process
}
